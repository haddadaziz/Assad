...
<body class="bg-gray-50 font-sans flex h-screen overflow-hidden">
    ...
    <aside class="w-64 bg-maroc-green text-white flex flex-col shadow-2xl">
        ...
        <nav class="flex-1 overflow-y-auto py-6">
            <ul class="space-y-2">
                <li>
                    <a href="?section=mes_visites"
                        class="flex items-center px-6 py-3 hover:bg-green-700 transition <?= $section == 'mes_visites' ? 'bg-green-900 border-l-4 border-yellow-400' : '' ?>">
                        <i class="fa-solid fa-map-location-dot w-6"></i>
                        <span class="font-medium">Mes Visites</span>
                    </a>
                </li>
                <li>
                    <a href="?section=reservations"
                        class="flex items-center px-6 py-3 hover:bg-green-700 transition <?= $section == 'reservations' ? 'bg-green-900 border-l-4 border-yellow-400' : '' ?>">
        ...
        <?php if ($section == 'mes_visites'): ?>
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="bg-white p-6 rounded-xl shadow-md border-t-4 border-maroc-red h-fit">
                    <h2 class="font-bold text-lg mb-4 text-gray-700"><i class="fa-solid fa-plus-circle mr-2"></i>Créer une
                        visite</h2>
                    <form method="POST" action="">
                        <div class="space-y-4">
                            <div>
                                <label class="block text-xs font-bold text-gray-500 uppercase mb-1">Titre de la
                                    visite</label>
                                <input type="text" name="titre" required placeholder="Ex: Safari Nocturne"
                                    class="w-full border p-2 rounded focus:ring-1 focus:ring-maroc-green">
                            </div>
                            ...
                            <button type="submit" name="creer_visite"
                                class="w-full bg-maroc-green hover:bg-green-800 text-white font-bold py-3 rounded transition shadow-md">
                                Publier la visite
                            </button>
                        </div>
                    </form>
                                    ...
                                    <?php foreach ($mes_visites as $v): ?>
                                        <tr class="border-b hover:bg-gray-50">
                                            ...
                                            <td class="p-4">
                                                ...
                                                <?php else: ?>
                                                    <span
                                                        class="bg-red-100 text-red-700 px-2 py-1 rounded-full text-xs font-bold">Annulée</span>
                                                <?php endif; ?>
                                            </td>
                                            <td class="p-4 text-right">
                                                <button
                                                    class="bg-blue-50 text-blue-600 px-3 py-1 rounded hover:bg-blue-100 text-xs mr-2"
