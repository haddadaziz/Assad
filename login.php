...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($user && password_verify($password,$user['motpasse_hash'])) {
        // Verifier si le compte est actif ou bloqué
        if ($user['statut'] == 1) {
        ...
        <div class="p-8">
            ...
            <form method="POST" action="">
                <div class="mb-4">
                    ...
                    <input type="email" name="email" required class="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:border-maroc-green">
                </div>

                <div class="mb-6">
                    <label class="block text-gray-700 text-sm font-bold mb-2">Mot de passe</label>
                    <input type="password" name="password" required class="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:border-maroc-green">
                    <a href="#" class="text-xs text-gray-500 hover:text-maroc-red float-right mt-1">Mot de passe oublié ?</a>
                </div>

                <button type="submit" class="w-full bg-maroc-green text-white font-bold py-3 rounded hover:bg-green-800 transition duration-300">
                    Se connecter
                </button>
            </form>
            
            <p class="mt-4 text-center text-sm text-gray-600">
                Pas encore de compte ? <a href="register.php" class="text-maroc-red font-bold hover:underline">S'inscrire</a>
            </p>
